...
class Attachments extends Base
{
    ...
    public function upload()
    {
        ...
        if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] == 0) {
            ...
            if ($attachmentType == Entry::ATTACHMENT_TYPE_IMAGE) {
                $this->resizeAttachedImage($tmpFilesDir . '/' . $filename, $tmpImgsPreviewDir . '/' . $filename, 500, 500); //generate preview
                $this->resizeAttachedImage($tmpFilesDir . '/' . $filename, $tmpResizedImgsDir . '/' . $filename, 1920, 1200); //generate resized copy
    ...
    private function removeExpiredFiles($dir)
    {
    ...
    private function resizeAttachedImage($imagePath, $destinationPath, $width, $height)
    {
        try {
